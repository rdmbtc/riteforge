◇ injected env (15) from .env.local // tip: ◈ secrets for agents [www.dotenvx.com]
...
/**
 * @title MinimalToken (EIP-1167 Minimal Proxy Clone)
 * @dev Minimal clone contract for tokens - only 45 bytes deployed
 */
contract MinimalToken {
    ...
    function setup(
        ...
        implementation = 0x0000000000000000000000000000000000000001; // marker
        owner = owner_;
    ...
    modifier onlyOwnerOrMaster() {
        require(msg.sender == owner || msg.sender == masterContract, "Not authorized");
        _;
    }

    function mint(address to, uint256 amount) external onlyOwnerOrMaster {
        _totalSupply += amount;
        _balances[to] += amount;
    }

    function burn(address account, uint256 amount) external onlyOwnerOrMaster {
    ...
    function name() public view returns (string memory) {
        return string(abi.encode(_name));
    }

    function symbol() public view returns (string memory) {
        return string(abi.encode(_symbol));
    }
    ...
    function transfer(address to, uint256 amt) public returns (bool) {
        require(_balances[msg.sender] >= amt);
    ...
    function transferFrom(address from, address to, uint256 amt) public returns (bool) {
        require(_allowances[from][msg.sender] >= amt);
        require(_balances[from] >= amt);
...
// File contracts/RiteForgeToken.sol

// Original license: SPDX_License_Identifier: MIT
pragma solidity ^0.8.24;

/**
 * @title RiteForgeToken
 * @dev Standard ERC20 token with owner mint/burn capabilities
 */
contract RiteForgeToken is ERC20 {
    address public owner;
    address public masterContract;

    modifier onlyMasterOrOwner() {
        require(msg.sender == masterContract || msg.sender == owner, "Not authorized");
        _;
    }
    ...
    function mint(address to, uint256 amount) external onlyMasterOrOwner {
        _mint(to, amount);
    }

    function burn(address from, uint256 amount) external onlyMasterOrOwner {
        _burn(from, amount);
    }
}


// File contracts/RiteForgeMaster.sol
